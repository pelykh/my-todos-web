...
import { ArrowLeft, Ellipsis, LayoutTemplate, Trash2 } from 'lucide-react'
...
export const Route = createFileRoute('/template/$taskId')({
...
function TemplatePage() {
	...
	const [task] = useTaskWithProject(taskId)
	...
	function handleBack() {
		navigate({ to: (return_to ?? '/templates') as never })
	}
	...
	function handleConvertToNextAction() {
		if (!task) return
		const tags = (task.tags ?? []).filter((t) => t !== 'template')
		editTask(task.id, { tags, status: 'next_action' })
			...
			>
				...
				>
					...
					<div className="flex gap-2 items-center flex-wrap">
						<Badge variant="light" color="teal" size="md">{t('templates')}</Badge>
						<BadgeSelect

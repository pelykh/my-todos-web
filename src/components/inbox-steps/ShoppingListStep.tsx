...
export function ShoppingListStep({ task, onAdvance }: Props) {
	...
	const { canSubmit, errors, handleSubmit } = useValidateTask({
		...
		fields: ['title'],
		onSubmit: () => {
			editTask(task.id, { status: 'backlog', tags: ['shopping_list'] })
			onAdvance()
		},
	})
	...
	return (
		<Stack gap="md" align="center">
			...
			<Text size="sm" c="dimmed" w="100%">
				{t('processShoppingListHint')}
			</Text>
			<TaskForm task={task} fields={['title']} />
			{errors.length > 0 && (

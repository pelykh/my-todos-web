...
export const filterStore = createStore<FilterStore>()(
  persist(
    (set) => ({
      ...
      actions: {
        setContext: (context) =>
          set((s) => ({ filters: { ...s.filters, context: context ?? undefined } })),
        toggleToday: () =>
          set((s) => ({ filters: { ...s.filters, isImportant: !s.filters.isImportant } })),
        activateToday: () =>
          set({ filters: { isImportant: true } }),
        setMaxMinutes: (maxMinutes) =>
          set((s) => ({ filters: { ...s.filters, maxEstimatedMinutes: maxMinutes  } })),
